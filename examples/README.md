...
## Story 3–5 production governance inputs

Production additionally requires `availability_policy`, a non-empty `approved_kubernetes_versions` containing the requested version, at least two system-pool zones, and NAP `network.capacity` when NAP is selected. `maintenance.auto_upgrade.not_allowed` and `maintenance.node_os.not_allowed` carry approved freeze windows. Development may keep the approved version set empty and does not require NAP capacity declarations because the production capacity gate is fail-closed only for production.
